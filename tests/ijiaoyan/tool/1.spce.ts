...
async function traverseFolders(page, folderSelector) {
  ...
  if (tableRow) {
    ...
    if (!hasSubFolders) {
      ...
      if (handout_row.length > 0) {
        for (const row of handout_row) {
          ...
          // await page.getByText('zhangq测试').click();
          // await page.getByText('106-能力提高').click();
          await page.click('.ant-breadcrumb-link:nth-last-child(2)');
        ...
        if (breadcrumbLinks.length >= 2) {
            ...
            console.warn('面包屑链接数量不足，无法点击倒数第二个');
        }
    ...
    console.warn('列表中没有数据，未找到 tr.ant-table-row 元素，此文件夹下无数据');
    // await page.getByText('测试专用公共云盘').click();
    // await page.getByText('zhangq测试').click();
    // await page.getByText('106-能力提高').click();
    // await page.goBack();
    ...
    // await page.waitForTimeout(3000);
    await page.click('.ant-breadcrumb-link:nth-last-child(2)');
    await page.click('.ant-breadcrumb-link');
    await page.waitForTimeout(1000);
  }
